...
```bash
uv run --group rlds scripts/tools/compute_norm_stats.py --config-name pi05_full_droid_finetune --max-frames 10_000_000
```

Run training:
```bash
XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 uv run --group rlds scripts/train/train.py pi05_full_droid_finetune --exp-name=my_experiment --overwrite
```
...
To launch training:
```
uv run scripts/train/train.py pi05_droid_finetune --exp-name=my_experiment --overwrite
```
