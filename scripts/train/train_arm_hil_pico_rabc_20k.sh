...
CONFIG_NAME="${CONFIG_NAME:-arm_pi05_lite28_tristate_rabc_from_lite0030_20000}"
...
EXP_NAME="${EXP_NAME:-arm_hil_pico_v21_20260704_tristate_rabc_from_lite0030_29999_bs256_fsdp${FSDP_DEVICES}_w${NUM_WORKERS}_lr5e-6_20k_${TS}}"
LOG_FILE="${LOG_FILE:-logs/${EXP_NAME}.log}"
SCREEN_NAME="${SCREEN_NAME:-openpi_arm_hil_pico_fsdp${FSDP_DEVICES}_${TS}}"
...
{
  echo "=== train_arm_hil_pico_rabc_20k ==="
  echo "date=\$(date -Is)"
...
exec "$PYTHON_BIN" scripts/train/train_arm.py "$CONFIG_NAME" \\
  --exp-name="$EXP_NAME" \\
  --fsdp-devices="$FSDP_DEVICES" \\
  --num-workers="$NUM_WORKERS" >> "$LOG_FILE" 2>&1
EOF
chmod +x "$RUN_SCRIPT"
